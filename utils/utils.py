...
def create_models(
    ...
    if train_inputs is not None and train_targets is not None:
        # Number of models is based on the length of the first element in train_targets
        num_models = train_targets.shape[1]

        # Generate indices list where each inner list contains the index and the range of values
        indices = [[idx_model] + list(range(num_models, num_models + idx_model + 1)) for idx_model in range(num_models)]

        # Print the generated indices
        print('indices: ', indices)

        # Initialize models based on the generated indices
        models = []
        for idx_model in range(num_models):
            input_indices = indices[idx_model]
            print(f'Model {idx_model}: input_indices = {input_indices}')

            # Ensuring indices are within the bounds of train_inputs columns
            input_indices = [i for i in input_indices if i < train_inputs.shape[1]]
            print('input_indices: ', input_indices)
            model_inputs = train_inputs[:, input_indices]
            model_targets = train_targets[:, idx_model]

            print(f'Model {idx_model}: model_inputs.shape = {model_inputs.shape}')
            print(f'Model {idx_model}: model_targets.shape = {model_targets.shape}')

            model = ExactGPModelMonoTask(model_inputs, model_targets, len(input_indices))
            models.append(model)
    ...
    for idx_model in range(num_models):
        ...
        if type(params) == dict:
            ...
            print(f'model params init {idx_model} ', params)

        elif type(params) == list:
            print(f'params {idx_model}', params[idx_model])
...
class LivePlotSequential:
    ...
    def update(self, obs, action, cost, info_dict=None):
        ...
        if info_dict is not None:
            ...
            np.nan_to_num(np.asarray(mean_costs_pred), copy=False, nan=-1, posinf=-1, neginf=-1)
            np.nan_to_num(np.asarray(mean_costs_std_pred), copy=False, nan=99, posinf=99, neginf=99)
            np.nan_to_num(np.asarray(states_pred), copy=False, nan=-1, posinf=-1, neginf=-1)
            np.nan_to_num(np.asarray(states_std_pred), copy=False, nan=99, posinf=99, neginf=0)
            np.nan_to_num(np.asarray(costs_pred), copy=False, nan=-1, posinf=-1, neginf=-1)
            np.nan_to_num(np.asarray(costs_std_pred), copy=False, nan=99, posinf=99, neginf=0)
